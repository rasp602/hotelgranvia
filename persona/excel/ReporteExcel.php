...
if ($campo!="") {
    $where="where rutPersona LIKE'%".$campo."%'";
...
if ($desde!="" && $hasta!="" && $campo=="" && $idEmpresa=="" && $genero=="") {
...
$sql ="SELECT  persona.idPersona,
        ...
        $where ORDER by persona.idPersona ASC";
	...
	$objPHPExcel->getActiveSheet()->setCellValue('B1', 'PERSONAS REGISTRADAS');
	...
	$objPHPExcel->getActiveSheet()->setCellValue('B6', 'NOMBRE PERSONA');
	$objPHPExcel->getActiveSheet()->getColumnDimension('C')->setWidth(10);
	$objPHPExcel->getActiveSheet()->setCellValue('C6', 'GENERO');
	$objPHPExcel->getActiveSheet()->getColumnDimension('D')->setWidth(20);
	$objPHPExcel->getActiveSheet()->setCellValue('D6', 'EMPRESA');
	$objPHPExcel->getActiveSheet()->getColumnDimension('D')->setWidth(20);
	$objPHPExcel->getActiveSheet()->setCellValue('E6', 'F.CREADO');
	
	//Recorremos los resultados de la consulta y los imprimimos
	while($rows = $resultado->fetch_assoc()){
		$objPHPExcel->getActiveSheet()->getStyle('A'.$fila.':'.'G'.$fila)->applyFromArray($estiloTituloColumnasResultados);

		$objPHPExcel->getActiveSheet()->setCellValue('A'.$fila, $rows['rutPersona']);
		$objPHPExcel->getActiveSheet()->setCellValue('B'.$fila, $rows['nombresPersona']." ".$rows['apellidoPersona1']);
		$objPHPExcel->getActiveSheet()->setCellValue('C'.$fila, $rows['genero']);
		$objPHPExcel->getActiveSheet()->setCellValue('D'.$fila, $rows['nombreEmpresa']);
		$objPHPExcel->getActiveSheet()->setCellValue('E'.$fila, $rows['fechaCreado']);
...
header('Content-Disposition: attachment;filename="personas registradas.xls"');
