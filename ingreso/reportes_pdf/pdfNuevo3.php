<?php
    ...
    $bg_descanso = '#FF6969'; //Color de fondo en los descansos
    $tcolor_descanso ='#fff'; //Color del texto en los descansos
    ...
    {   
        while($row = mysqli_fetch_assoc($result)) {
            ...
            $sql3 ="SELECT RIGHT(e.fecha_inicio,2) * 1 AS fecha_inicio, RIGHT(e.fecha_fin,2) * 1 AS fecha_final FROM eventoscalendar e
                    WHERE e.idTrabajador = ". $idTrabajador ." AND e.fecha_inicio BETWEEN DATE_ADD(DATE_ADD(LAST_DAY('". $mes ."'), INTERVAL 1 DAY),INTERVAL -1 MONTH)
                    AND LAST_DAY('". $mes ."') ORDER BY e.idTrabajador, e.fecha_inicio ASC ;";            
            $result3 = mysqli_query($conn, $sql3);            
            $desc_act = 0;
            unset($li_descanso); 
            $li_descanso = array();
            while($row3 = mysqli_fetch_assoc($result3)) { 
                for($h = $row3['fecha_inicio']; $h <= $row3['fecha_final']; $h++){    
                    array_push($li_descanso,$h);                                                    
                }
            }
            if(count($li_descanso) == 0){
                array_push($li_descanso,0);
            }
                    ...
                    for($j = $jaux; $j <= $num_days; $j++,$jaux++){
                        if($row2['fechac'] == $j){      
                            ...
                            if(intval($row2['fechaEntradaT']) != 0 ){                               
                                ...
                                $days_count++;          
                                $vertical_sum[($j-1)] += 1;  
                                if($j == $li_descanso[$desc_act]){
                                    if($desc_act < (count($li_descanso) - 1)){
                                        $desc_act++;
                                    }
                                }      
                                break;
                            }else if($j == $li_descanso[$desc_act]){                                                                                                
                                $tds .= '<td style="background-color: '. $bg_descanso .'; color: '. $tcolor_descanso .'; font-size: 15px; font-weight: bold" class="t-center">X</td>';                                                                                                                                             
                                $jaux++;      
                                if($desc_act < (count($li_descanso) - 1)){
                                    $desc_act++;
                                }
