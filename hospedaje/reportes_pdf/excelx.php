...
$idEmpresa = $_GET['idEmpresa'];
    ...
    if($idEmpresa != 1)
    {
        ...
        $precio_doble = $row['contratoEmpresa'];
        $precio_sencillo = $row['contratoEmpresa1'];
    ...
    p.rutPersona, ha.nHabitacion, ho.idPersona, ho.tipoHabitacion,
count(*) as resum
    FROM hospedaje ho
    ...
    INNER JOIN resumenhospedaje r ON r.idHospedaje = ho.idHospedaje
  WHERE p.idEmpresa = '. $idEmpresa .' and r.FechaR BETWEEN "'. $dia_ini .'" AND "'. $dia_fin .'"'.'   
   group by ho.idHospedaje having count(*)>1 ORDER by ho.idHabitacion
        ';
        ...
        {  
            while($row = mysqli_fetch_assoc($result)) 
            {
                ...
                if($row['tipoHabitacion'] == 'D')
                    $ContratoEmpresa = $precio_doble;
                else if($row['tipoHabitacion'] == 'S')
                    $ContratoEmpresa = $precio_sencillo;
