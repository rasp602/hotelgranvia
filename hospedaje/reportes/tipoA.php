...
if ($descripcion!="") {/*
    $where="where persona.rutPersona LIKE'%".$descripcion."%' OR persona.nombresPersona  LIKE'%".$descripcion."%' OR apellidoPersona1 LIKE'%".$descripcion."%' OR persona.nombresPersona  LIKE'%".$descripcion."%' and apellidoPersona1 LIKE'%".$descripcion."%' ";
 //echo "Busca descripcion";*/
}
...
if ($idHotel!="" && $idHabitacion=="" && $idCama=="" && $idEmpresa=="" && $estado=="" && $desde=="" && $hasta=="") {
    $where="where hotel.idHotel ='".$idHotel."' ";
 echo "2.-Busca Hotel solo"; 
...
if ($idHotel!="" && $idHabitacion!=""&& $idCama=="" && $idEmpresa=="" && $estado=="" && $desde=="" && $hasta=="") {
    $where="where hotel.idHotel ='".$idHotel."' and habitacion.idHabitacion ='".$idHabitacion."' ";
     echo "3.-Busca Hotel y Habitacion "; 

  }

  if ($idHotel!="" && $idHabitacion!="" && $idCama!="" && $idEmpresa=="" && $estado=="" && $desde=="" && $hasta=="") {
    $where="where hotel.idHotel ='".$idHotel."' and habitacion.idHabitacion ='".$idHabitacion."' AND cama.idCama ='".$idCama."' ";
  echo "4.-Busca hotel, habitacion y Cama"; 
}

   if ($idEmpresa!="" && $idHotel!="" && $idHabitacion=="" && $idCama=="" && $estado=="" && $desde=="" && $hasta=="" ) {
    $where="where hotel.idHotel ='".$idHotel."' and persona.idEmpresa ='".$idEmpresa."' ";
  echo "5.-Busca hotel y empresa "; 

   }
   if ($estado!="" && $idHotel!="" && $idHabitacion=="" && $idCama=="" && $idEmpresa==""  && $desde=="" && $hasta=="") {
    $where="where hotel.idHotel ='".$idHotel."' and hospedaje.estado ='$estado'";
  echo "6.-Busca hotel y Estado"; 
}
  ...
  echo "8.-.-Busca hotel, habitacion,Cama,empresa y estado"; 
   ...
   if ($idHotel!="" && $idEmpresa!=""  && $estado!="" && $idHabitacion=="" && $idCama==""  && $desde=="" && $hasta=="" ) {
...
$query = mysqli_query($con,"SELECT 
        ...
        $where ORDER by habitacion.nHabitacion asc LIMIT $offset,$per_page");
		...
		<table class="table table-condensed table-striped table-bordered table-hover" id="tabla">
            ...
            <td class="contenidoTabla">
              ...
              <td> <a onclick="javascript:return confirm('¿Seguro de eliminar este registro?');" href="?c=hospedaje&a=Eliminar&idHospedaje=<?php echo $row['idHospedaje']?>" class="glyphicon glyphicon-remove"></a></td>
     			</tr>
			<?php
			 }
			?>
