...
for ($i = 1; $i <= $numfilas; $i++) {
    ...
    $test = "SELECT COALESCE((SELECT tipo_evento FROM eventoscalendar WHERE idTrabajador = ". $fila['idTrabajador'] ." AND '". $fecha_original ."' BETWEEN fecha_inicio AND fecha_fin LIMIT 1),'Descanso') AS condicion ";
    $query4 = mysqli_query($conn, $test);
    $result4 = mysqli_fetch_array($query4);
    $condicion =  $result4['condicion'] . "<br>";
